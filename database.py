...
class DatabaseManager:
    ...
    def get_monthly_kudos_count(self, user: str, month: int, year: int) -> int:
        """Get the number of kudos sent by a user in a specific month"""
        sql = """
    ...
    def get_monthly_leaderboard(self, month: int, year: int):
        ...
        with self.get_connection() as conn:
            with conn.cursor() as cursor:
                # Get top senders
                cursor.execute(sender_sql, (month, year))
                top_senders = cursor.fetchall()
                
                # Get top receivers
                cursor.execute(receiver_sql, (month, year))
    ...
    def get_user_stats(self, user: str):
        ...
        with self.get_connection() as conn:
            with conn.cursor() as cursor:
                cursor.execute(sent_sql, (user,))
                total_sent = cursor.fetchone()[0]
                
                cursor.execute(received_sql, (user,))
                total_received = cursor.fetchone()[0]
                
                cursor.execute(monthly_sent_sql, (user,))
